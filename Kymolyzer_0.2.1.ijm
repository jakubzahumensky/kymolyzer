...
function dialogWindow(folder){
	...
	Dialog.createNonBlocking("Kymolyzer");
		...
		Dialog.addString("Channel display:", "Cyan, Magenta", 20);
...
function prepareROIs(img_title){
	roiManager("reset");
	roiManager("Open", dir_ROIs + cleanTitle(img_title) + "-RoiSet.zip");
...
function analyzeKymograms(){
	prepare();
	background = measureBackground(title);
...
//			filterKymogramsFourier(kymogram_title);
			analyzeCorrelation(kymogram_title);
...
function filterKymogramsFourier(kymogram_image_name){
	x = 3;
...
function analyzeCorrelation(kymogram_image_name){
	...
	for (x = 1; x < width-1; x++){
		...
		profile_CV = Array.concat(profile_CV, stdDev/(mean-background));
	}
	Array.getStatistics(profile_CV, min, max, mean, stdDev);
	kymogram_CV = mean;
	print(current_channel + "," + kymogram_image_name + "," + kymogram_CV);
...
function measureBackground(image_title){
