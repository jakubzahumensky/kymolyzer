...
initial_folder = "D:/Yeast/EXPERIMENTAL/macros/JZ-IJ-004 - Kymolyzer/test_images/JZ-M-072/250411/";
...
function printResultsHeader(){
    ...
    print("[" + temporary_results_file + "]","# CV - coefficient_of_variation" + "\n");
    print("[" + temporary_results_file + "]","#" + "\n"); // empty line that is ignored in bash and R
}

function printColumnNames(){
    column_names = "exp_code,BR_date,"
        + naming_scheme + ",background,frame_interval,cell_no"
...
//			CV = analyzeVariationAndCorrelation(kymogram, kymogram_width, kymogram_height, background);
//			reconstructed_kymogram = mergeFilteredKymograms(kymogram_title);
//           CV2 = analyzeVariationAndCorrelation(reconstructed_kymogram, kymogram_width, kymogram_height, 0);
            if (analyze_overlap == true){
...
function mergeFilteredKymograms(kymogram_ID){
	merged_kymogram = dir_kymograms_image_filtered + kymogram_ID + "-merged.tif";
	open(merged_kymogram);
	run("Properties...", "channels=1 slices=3 frames=1");
	run("Z Project...", "projection=[Sum Slices]");
	image_title = dir_kymograms_image_filtered + kymogram_ID + "-merged-flat.tif";
	saveAs("TIFF", image_title);
	close("*");
	return image_title;
}
 ...
 */
function analyzeVariationAndCorrelation(kymogram_image, width, height, background_correction){
	open(kymogram_image);
    image_title = getTitle();
    getDimensions(temp_image_width, temp_image_height, temp_image_channels, temp_image_slices, temp_image_frames);
    if (current_channel <= temp_image_channels)
    	Stack.setChannel(current_channel);
    line_width = 3;
    ...
    for (x = 1; x < width-1; x++){
        ...
        CV_current_column = stdDev/(mean - background_correction);
        if (isNaN(CV_current_column))
        	CV_current_column = 0;
        CV_array = Array.concat(CV_array, CV_current_column);
