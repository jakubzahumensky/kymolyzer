...
function analyzeKymograms(res_file, proc_file){
	...
	for (j = 0; j < kymogram_list.length; j++){
		...
		if (!endsWith(kymogram, "-excluded.tif")){
			...
			average_speeds = averageSpeedAndLifetime(kymogram_title, kymogram_width, kymogram_height); // includes mean_lifetime
/*
...
function averageSpeedAndLifetime(image_title, width, height){
	...
	average_speeds = newArray();
	average_lifetimes = newArray();
	for (k = 0; k <= 2; k++){
		time_stretch = 10;
		filtered_image = dir_kymograms_image_filtered + kymogram_title + "-" + filters[k] + ".tif";
		extractKymogramTraces(filtered_image);
		average_speeds[k] = averageSpeed();
		average_lifetimes[k] = averageLifetime();
		close("Results");
		close("*");
	}
	output_array = Array.concat(average_speeds, average_lifetimes);
	return output_array;
...
/* Measure average speed of all traces in the kymogram. */
function averageSpeed(){
	speeds_array = newArray();
	for (i = 0; i < nResults; i++){
		length = getResult("Branch length", i);
		if (length > pixelWidth/2){
			Delta_x = (getResult("V2 x", i) - getResult("V1 x", i))*pixelWidth*1000; // lateral displacement in nm
			Delta_y = (getResult("V2 y", i) - getResult("V1 y", i))*frame_interval/time_stretch; // time in seconds
			speed = Delta_x/Delta_y; // get raw speed in pixels per frame interval
...
/* Measure average lifetime of traces that either start and finish within the kymogram or span the whole kymogram. */
function averageLifetime(){
	mean_lifetimes = newArray();
	lifetimes_array = newArray();
	for (i = 0; i < nResults; i++){
		length = getResult("Branch length", i);
		if (length > pixelWidth/2){
			trace_start = getResult("V2 y", i);
			trace_finish = getResult("V1 y", i);
		}
		stretched_height = height*time_stretch - 1; // the skeleton always ends at least 1 pixel before end
		lifetime_raw = abs(trace_finish - trace_start);
		condition1 = ((0 < stretched_height - trace_start) && (stretched_height - trace_start < stretched_height)) && ((0 < stretched_height - trace_finish) && (stretched_height - trace_finish < stretched_height));
		condition2 = (lifetime_raw == stretched_height);
		if (condition1 || condition2){
			lifetime = lifetime_raw*frame_interval/time_stretch; // lifetime in seconds
			lifetimes_array = Array.concat(lifetimes_array, lifetime);
		}
	}
...
function printResults(){
	...
	kymogram_results = parents[0] + "," + parents[1] // [0] - experiment code, [1] - biological replicate date
		...
		+ "," + arrayToString(average_speeds) + "," + CV;
